document.addEventListener('DOMContentLoaded', function () {
    ...
    solveBtn.addEventListener('click', async () => {
        ...
        statusText.innerText = "Analyzing grid...";
        ...
        if (!tab.url.includes("nytimes.com/puzzles/sudoku")) {
            // Fallback for allowing testing on other URLs if needed, but primarily strict for production
            stopTimer();
            statusBox.className = "status-box error";
            statusText.innerText = "Error: Please open NYT Sudoku.";
            resetButton();
            return;
        }
        ...
        try {
            const response = await chrome.tabs.sendMessage(tab.id, { action: "solve_sudoku" });
            ...
            if (response && response.status === "success") {
                ...
                throw new Error(response ? response.message : "Script error");
            }
        } catch (error) {
            console.error(error);
            stopTimer();
